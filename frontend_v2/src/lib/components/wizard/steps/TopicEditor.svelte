<script lang="ts">
	...
	function shouldFilter(item: MediathekItem): boolean {
		...
		return title.includes('audiodeskription') || title.includes('gebärdensprache');
	}
...
<div class="topic-editor">
	...
	<div class="flex flex-col gap-1">
		<div class="flex items-center justify-between">
			<label class="text-sm font-medium text-text-secondary" for="topic-input">Topics</label>
			<span class="text-xs text-text-tertiary">{topics.length} Topic{topics.length !== 1 ? 's' : ''}</span>
		</div>
	...
	<details class="mt-6 rounded-lg border border-border bg-bg">
		...
		<div class="px-4 pb-4 text-sm">
			...
			<ol class="list-decimal list-inside space-y-2">
				<li>
					Öffne <a href="https://mediathekviewweb.de" target="_blank" rel="noopener" class="text-accent hover:underline">MediathekViewWeb</a>
				</li>

...
<div class="title-rules-editor">
	<h2 class="text-2xl font-bold mb-2">Titel-Regeln</h2>
		...
		<div class="mb-6">
			...
			<div class="space-y-2">
				{#each titleRules as rule, index}
					<div class="flex items-center gap-2 bg-base-100 p-3 rounded-lg">
						<span class="badge" class:badge-secondary={rule.type === 'static'} class:badge-accent={rule.type === 'regex'}>
							{index + 1}. {rule.type === 'static' ? 'Statisch' : 'Regex'}
						</span>

						<div class="flex-1 flex items-center gap-2 min-w-0">
							{#if rule.type === 'static'}
								<code class="bg-base-200 px-2 py-1 rounded text-sm">"{rule.value}"</code>
							{:else}
								<code class="bg-base-200 px-2 py-1 rounded text-sm">{rule.pattern}</code>
								<span class="text-sm text-base-content/60">aus {rule.field}</span>
							{/if}
						</div>
			...
			{#if constructedExample}
				<div class="mt-4 p-3 bg-success/10 rounded-lg text-sm">
					<span class="text-base-content/70">Beispiel:</span>
	...
	<div class="card bg-base-100">
		<div class="card-body">
			<h3 class="card-title text-lg">Neue Regel hinzufügen</h3>

			<!-- Rule type tabs -->
			<div class="tabs tabs-boxed w-fit">
				<button
					class="tab"
					class:tab-active={newRuleType === 'regex'}
				...
				<button
					class="tab"
					class:tab-active={newRuleType === 'static'}
			...
			{#if newRuleType === 'static'}
				<!-- Static rule form -->
				<div class="form-control mt-4 max-w-sm">
					<label class="label" for="static-value">
						<span class="label-text">Text, der angehängt wird</span>
					</label>
					<input
						id="static-value"
						type="text"
					...
					/>
					<label class="label">
						<span class="label-text-alt text-success">Statische Regeln können nie fehlschlagen</span>
					</label>
				</div>
			{:else}
				<!-- Regex rule form -->
				<div class="space-y-2 mt-4 max-w-sm">
					<div class="form-control">
						<label class="label" for="regex-field">
					...
					<div class="form-control">
						<label class="label" for="regex-pattern">
							<span class="label-text">Regex-Muster</span>
							{#if patternValidation.error}
								<span class="label-text-alt text-error">{patternValidation.error}</span>
							{/if}
						</label>
						<input
							id="regex-pattern"
							...
							class:input-error={newRegexPattern && !patternValidation.valid}
							placeholder="z.B. (?<=: )(.+)$"
	...
	<div class="collapse collapse-arrow bg-base-100 mt-6">
		...
		<div class="collapse-content text-sm">
			...
			<div class="alert alert-info mb-4">
				...
				<span>
					<strong>Wichtig:</strong> Nur der <u>erste Klammerinhalt</u> <code class="bg-base-200 px-1 rounded">(...)</code> wird extrahiert!
				</span>
			</div>
			...
			<div class="overflow-x-auto">
				<table class="table table-sm">
					...
					<tbody>
						<tr>
							<td><code class="bg-base-200 px-1 rounded">(.*)</code></td>
							<td>Alles</td>
							<td>"Hallo Welt" → "Hallo Welt"</td>
						</tr>
						<tr>
							<td><code class="bg-base-200 px-1 rounded">(\d+)</code></td>
							<td>Zahlen</td>
